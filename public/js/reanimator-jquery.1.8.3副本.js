...
(function( window, undefined ) {
	var native_setTimeout = global.setTimeout;
	var native_setInterval = global.setInterval;
	var native_Date = global.Date;
...
jQuery.extend({
	...
	ready: function( wait ) {
		...
		if ( !document.body ) {
			return native_setTimeout( jQuery.ready, 1 );
		}
	...
	now: function() {
		return ( new native_Date() ).getTime();
	}
});
...
jQuery.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		if ( document.readyState === "complete" ) {
			...
			native_setTimeout( jQuery.ready, 1 );
				...
				(function doScrollCheck() {
					if ( !jQuery.isReady ) {
						...
						try {
							...
							return native_setTimeout( doScrollCheck, 50 );
						}
...
jQuery.fn.extend({
	...
	delay: function( time, type ) {
		...
		return this.queue( type, function( next, hooks ) {
			var timeout = native_setTimeout( next, time );
			hooks.stop = function() {
			...
			if ( !handlers ) {
				...
				if ( !special.setup || special.setup.call( elem, data, namespaces, eventHandle ) === false ) {
					...
					if ( elem.addEventListener ) {
						elem.origElementEventListener( type, eventHandle, false );
	...
	ajax: function( url, options ) {
		...
		if ( !transport ) {
			...
			if ( s.async && s.timeout > 0 ) {
				timeoutTimer = native_setTimeout( function(){
					jqXHR.abort( "timeout" );
				}, s.timeout );
			}
...
jQuery.fx.timer = function( timer ) {
	if ( timer() && jQuery.timers.push( timer ) && !timerId ) {
		timerId = native_setInterval( jQuery.fx.tick, jQuery.fx.interval );
	}
};
...
function getCaptureOnHandlerFn(fn) {
  return function (event) {
    ...
    if(typeof originalEvent._reanimator.replay =='undefined'){
        originalEvent._reanimator.replay = true;
        originalEvent._reanimator.entry.fun.push(fn.toString());
        Reanimator.log.events.push(originalEvent._reanimator.entry);
    }else{
          var len = Reanimator.log.events.length;
          Reanimator.log.events[len-1].fun.push(fn.toString());
    }
    /*
    ...
    /*if (originalEvent && originalEvent.type && (
      ...
      //console.log("jquery "+ JSON.stringify(entry.details));
...
Reanimator.plug('jquery', {
  ...
  //modified by guoquan
  //in capture phase we only need capture events, and in replay phase we override the jquery.on function;
